...
# and `pip install optimization-benchmark` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "optimization-benchmark", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n optimization-benchmark -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "optimization-benchmark", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "optimization-benchmark", "YOUR_CONSOLE_SCRIPT"]
